...
- Every skill working with code starts with `recall(bank=<repo>)` and ends with `retain(bank=<repo>)`.
...
| KAIROS-01 | Every cloned repo has .agents/ from kairos. |
...
- Override: "razona" extends responses. "rapido" accelerates them.
