...
**CONFIRMED 2026-08-16 — reflect on giant banks SATURATES the MCP server; pivot to direct retain.** New failure mode: issuing reflect on the two giant banks (personal-buffer 9,228 facts, hermes 2,822) saturated the MCP server — subsequent reflect calls timed out one after another ("MCP server saturado por los reflect en banks grandes"). Exports had completed fine; it was the reflect phase that collapsed. Recovery that worked: (1) STOP issuing reflect calls immediately; (2) finish the run with direct `retain` for every remaining bank, composing the summary manually from the exported JSON (fact count + "sin actividad en 24h" + key state/decisions visible in the export); tags always `["daily-summary", "YYYY-MM-DD", "BANK_ID"]`; (3) wait a cooldown after the timeouts before retrying anything. Retains issued 4-5 per batch still returned `{"status":"accepted","operation_id":...}` even while the server was strained — **retain is the cheap operation, reflect is the load-sensitive one.** Lesson: if a reflect times out mid-run, don't keep hammering reflect; pivot the rest of the run to direct retain and treat reflect as optional per-bank.
...
| **Reflect on giant banks saturates the MCP server (2026-08-16)** | Issuing reflect on the two biggest banks (personal-buffer ~9.2K facts, hermes ~2.8K facts) caused cascading timeouts on ALL subsequent reflect calls. Exports unaffected — it's specifically the reflect phase. **Mitigation:** if a reflect times out, STOP issuing reflects; complete the remaining banks with direct retain (manual summary from exported JSON, tags `["daily-summary", YYYY-MM-DD, BANK_ID]`), then wait a cooldown before retrying reflect sequentially. Retains batch fine 4-5 at a time even under strain — reflect is the load-sensitive op, retain is cheap. |
