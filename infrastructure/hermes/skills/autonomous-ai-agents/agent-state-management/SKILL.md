...
- **Reflect saturation on giant banks (2026-08-16)**: Issuing reflect on the two largest banks (personal-buffer ~9.2K facts, hermes ~2.8K facts) saturated the MCP server and caused cascading timeouts on ALL subsequent reflect calls — exports unaffected, it's specifically the reflect phase. Pivot: if a reflect times out, STOP reflecting; finish the remaining banks with direct retain (manual summary composed from the exported JSON: fact count + activity state + pending decisions, tags `["daily-summary", YYYY-MM-DD, BANK_ID]`). Retains batch 4-5 per turn even under strain; reflect is the load-sensitive op, retain is cheap. Full detail in `hermes-sync-configure/references/bank-sync-execution.md`.
