...
- **[ROUTE-01] Deterministic routing via bridge injection:** `patch-bridge.sh` modifies bridge.js to inject a `=== PROFILE ACTIVATION: <name> === {full profile SOUL.md} === END PROFILE ===` block into every WhatsApp group message that has a configured profile. The bridge reads `~/.hermes/profiles/<name>/SOUL.md` in each message — no caching. The LLM adopts this as its identity via SOUL.md RULE 0. No LLM judgment in routing — the bridge resolves the profile in code.
