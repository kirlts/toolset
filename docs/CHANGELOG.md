...
### Changed
- **`/okos-mapa/` pasa de la maqueta v5 a la Torre v9 con el contrato vivo embebido** (2026-08-10). El `index.html` de `/opt/toolset/landing/okos-mapa/` ahora es la Torre OKOS v9 (857 KB): 292 piezas · 222 cintas · 4 días de historia, generado desde el repo `okos-mapa` (`prototype/generar_digest.py` produce el bloque de datos desde `data/mapa.json`; `prototype/ensamblar_v9.py` arma el HTML desde el esqueleto v8 con reemplazos de conteo exacto). `support.js` e `image-slot.js` quedan tal cual; **ni Caddyfile ni Funnel se tocaron**. Respaldo del anterior en `index.html.bak.1786393122`. Verificado desde fuera de la tailnet: 200 con el tamaño exacto del archivo (876 962 bytes), `/okos-mapa/no-existe.js` → 404, `/health` sin regresión, y un navegador real contra la URL pública renderiza sin errores de consola. **Cambio de exposición que la v5 no tenía**: la v9 embebe datos reales de la plataforma —nombres de espacios de clientes, cuotas y consumos por plan, evidencia literal de sondas—. Sigue público y sin `basic_auth` (decisión del usuario, igual que el 2026-08-08); si eso cambia, es una línea del Caddyfile. **Actualizado el mismo día con una pasada de rendimiento medida con perfil de CPU y tracing**: la cámara del gesto (rueda y paneo) vive en el compositor y el viewBox se asienta solo cuando el zoom acumulado supera ~1.4× o al reposar 450 ms — cero tormentas de raster por muesca de rueda—; la máscara SVG pasó de 222 aplicaciones (una por camino) a una sola sobre la capa, con el camino encendido duplicado en una capa sin máscara; el vestido de fichas es analítico (cero layouts forzados); y las animaciones se pausan durante el gesto. Bloqueo del hilo principal en 12 pasos de zoom con GPU: 0.46 s (antes ~8 s en la misma batería). **Segunda pasada el mismo día, tras observarse parpadeo y zonas sin pintar con el asentamiento diferido**: la cámara pasó a ser un transform CSS permanente sobre lienzo de layout fijo (la arquitectura de React Flow) — el viewBox no se toca nunca después del montaje, no existe el «commit» ni su tormenta de raster, y Chrome re-rasteriza asincrónico manteniendo visibles los tiles anteriores. Medido de nuevo con GPU: 0.24 s de bloqueo en los mismos 12 pasos, sin parpadeo y sin zonas invisibles. **Tercera pasada: el nivel 2.** Dos causas medidas: la máscara SVG de los caminos obligaba a una superficie intermedia del grupo entero en cada pasada de raster (quitarla llevó el reposo del nivel 2 de 12 a 51 fps en raster por software) — se reemplazó por un `clipPath` evenodd, geometría pura que pinta idéntico —; y las hormigas animaban `stroke-dashoffset` a 60 Hz, invalidando los tiles continuamente — ahora avanzan por pasos (steps(16), mismo movimiento) y se pausan durante el gesto. Nivel 2 medido en la URL pública, raster por software (peor caso): reposo 8.8→50 fps, paneo 8→26 fps, pasos de zoom 4.2→9.6 fps. **Ajuste posterior**: los 65 flujos por pasos saltaban SINCRONIZADOS (mismo ciclo de 1.6 s) — un tic al unísono cada 100 ms que se leía como parpadeo de las conexiones; cada flujo lleva ahora su propio desfase. Y el hover de un camino se enciende al POSARSE (130 ms), no al barrer el mouse por encima — el corredor de hit de ~20 px por 222 caminos hacía destellar las conexiones con cada pasada. **Cuarta pasada — física de cámara**: zoom con easing (cada muesca mueve el objetivo y la cámara lo persigue con retargeting exponencial), paneo con inercia y decaimiento al soltar, cámara acotada (al menos un cuarto del encuadre siempre muestra plano: el mundo ya no puede «desaparecer» paneando), montaje de contenido EN CAMINO durante el gesto (refresh cada 250 ms — nada aparece de golpe al asentarse), y las capas a opacidad 0 quedan `visibility:hidden` (las tarjetas invisibles de nivel 1 cazaban clics y hover sobre el suelo del nivel 2, y ya ni se pintan: reposo del nivel 2 en raster por software 47→59.6 fps, paneo 27→58.5). Con GPU: reposo + paneo con inercia + 10 pasos de zoom = CERO tareas largas del hilo principal. **Quinta pasada — acabado de diseño, sin tocar identidad ni contenido**: transiciones de micro-interacción en toda la UI fija (migas, chips, niveles, filas de panel, botones ⑂, tarjetas de zona, rótulos de categoría), profundidad real en los paneles laterales, scrollbars del sistema estilizadas, cursor grab/grabbing en el lienzo, entrada animada del menú, fade de carga de la app, y el tab del navegador con nombre («OKOS · Live Map») e ícono propio. Rendimiento intacto: nivel 2 a 59.6/57.5 fps en raster por software. Además `prefers-reduced-motion` respetado, `::selection` propio, y los rótulos de bottleneck/worse del nivel 1 calibrados CONTRA MEDICIÓN del ancho rendido: caben enteros en 7 de 8 casos (el octavo, Billing, es físicamente imposible en su tarjeta: elipsis limpia + hover con el texto completo). **Sexta pasada — el parpadeo real y la consola, de raíz**: el runtime DC clava las listas POR ÍNDICE, así que el montaje-en-camino insertaba fichas al medio y React reescribía todas las posteriores — un flash masivo cada 250 ms durante el gesto. Ahora las 292 fichas se montan UNA vez y la lista no cambia jamás dentro de un día/alcance: CERO mutaciones de DOM durante el paneo, medido con MutationObserver. Y los atributos SVG interpolados del template viajan con el prefijo `sc-camel-` del runtime (el navegador no valida atributos desconocidos): los 10 errores de consola del parseo previo a la hidratación desaparecieron — consola LIMPIA, verificada en la URL pública. **Séptima pasada — el mapa como producto de teléfono y detalles de luz**: soporte táctil completo (un dedo panea con la misma física e inercia del mouse; dos dedos pellizcan anclados al centro del gesto — verificado con eventos táctiles reales sobre la URL pública), doble clic acerca un paso con easing (Shift aleja), halo luminoso en la cadena encendida del Path Lit, y el chip de día se tiñe de ámbar cuando el mapa mostrado es histórico. **Octava pasada — medida EN LA MÁQUINA REAL (Intel Arc integrada, ventana visible)**: el nivel 2 en reposo corría a 7 fps por puro raster de adornos, y los redibujos llegaban «a destiempo» (tiles goteando). Tres respuestas: (1) las hormigas invalidan en 4 ráfagas por ciclo, no continuo; (2) MODO LITE ADAPTATIVO — el mapa se mide en reposo y si no sostiene ~35 fps apaga lo que re-rasteriza (hormigas quietas pero visibles, tarjetas planas, halo off), decidido en ~1.5 s y persistido en localStorage; (3) el zoom viaja como TRANSICIÓN CSS de transform coalescida (~7/s): el compositor la trata como animación acelerada — rasteriza una vez y escala la pantalla ENTERA junta, sin tiles a destiempo — y los vuelos de navegación usan el mismo mecanismo con crossfade de opacidad acompañado. Medido en la máquina real: reposo nivel 2 de 7→107 fps, paneo 38→59, pasos de zoom 13→32. **Novena pasada — la definitiva, a los píxeles reales del panel (2880×1800, escala 2)**: el lag restante del zoom no era el raster (asíncrono) sino que cada paso disparaba trabajo del hilo principal (crossfade, snap de nivel, vestido) cuyos COMMITS esperaban al raster de 5MP. Doctrina nueva: durante un gesto el hilo principal no hace NADA — solo transform al compositor; crossfade, snap, refresh y vestido esperan a 560 ms de silencio. Medido a dpr=2 (los píxeles exactos del notebook): reposo 109 fps, paneo 47→96, pasos de zoom 2.9→60, zoom continuo →81. Consola limpia, regresión funcional verde. También tres bugs de la v8: el panel de área ya no queda abierto tapando clics tras «Open the area», el botón ⑂ tiene un mínimo de 24 px en pantalla (medía 10), y los rótulos de bottleneck/worse del nivel 1 se encogen hasta caber con hover completo.
