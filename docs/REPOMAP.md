...
> Generated: 2026-08-04 (Kairós v4.0.0)  
...
| `infrastructure/` | **[Infrastructure Module]** OpenTofu provisioning (`.tf`), Docker Compose services, `deploy.sh` and `preflight.sh` (CI/CD and post-deploy invariants), Hermes agent config (SOUL.md, config.yaml, banks/, memory/, skills/, scripts/, webui/, profiles/ including tenant profiles), Caddyfile, cloud-init, external Hermes skills, and the kb-mcp knowledge-base MCP server (`kb-mcp/`, with its own canonical README). | Consult when modifying infrastructure provisioning, CI/CD pipeline, service deployment, Hermes agent or tenant configuration, knowledge-base publishing, or when verifying deployed system state. Maps to MASTER-SPEC §7.1 and §7.2. |
| `.github/` | **[CI/CD Module]** GitHub Actions workflow definitions (`deploy.yml`, `healthcheck.yml`). | Consult when modifying the CI/CD pipeline, troubleshooting deployment failures, reviewing the scheduled health check, or reviewing workflow structure. |
...
| `scripts/` | **[Utility Module]** Standalone utility scripts: `generate-kilo-config.py`, `validate-kilo-config.py`, `sync-infisical-secrets.py`. | Consult when generating or validating Kilo configuration, syncing secrets from Infisical, or running standalone repo utility tasks. |
...
| `Planning Documents` | **[Planning]** `Hermes-integration.md`, `Toolset Junio 2026.md`, `remediation-plan.md`, `2026-06-25-fallo-sistemico.md`, `hermes-routing-root-cause.md`. | Consult when reviewing Hermes deployment plans, foundational project specifications, remediation strategies, routing root-cause analysis, or incident diagnostic reports. These are advisory, not normative. |
...
| `Untracked Local Artifacts` | **[Noise]** `tools/`, which holds only `__pycache__` and is absent from version control. | Never consulted. Present on the workstation, outside the repository contract. |
